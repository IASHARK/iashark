(function (root, factory) {
  const api = factory(
    ...
    typeof require === 'function' ? require('./team-names') : root.IasharkTeamNames,
    typeof require === 'function' ? require('./venue') : root.IasharkVenue
  );
  if (typeof module === 'object' && module.exports) module.exports = api;
  root.IasharkMatchViewModel = api;
})(typeof globalThis !== 'undefined' ? globalThis : this, function (display, insights, marketLabels, teamNames, venueLib) {
  'use strict';
  ...
  // Nom d'affichage (lib/team-names.js, audit SEO du 19/09/2026 : « América »
  // et non « Club America ») ; le nom du flux reste dans les donnees.
  const shownName = value => (teamNames && teamNames.displayName ? text(teamNames.displayName(value)) : null);
  const team = value => ({ id: value && value.id != null ? value.id : null, name: shownName(value) || text(value && value.n) || 'Équipe', logo: value && value.id ? `https://media.api-sports.io/football/teams/${value.id}.png` : null });
  const teamText = value => { const v = text(value); return v && teamNames && teamNames.displayName ? text(teamNames.displayName(v)) || v : v; };

  // ---------------------------------------------------------------------------
  // OUVERTURE DES ANALYSES DE MATCHS TERMINES (decision du proprietaire,
  // 20/09/2026 ; docs/SPEC_RESULTATS_HIER.md, lot R4).
  //
  // Une analyse dont le match est TERMINE n'a plus aucune valeur de pari :
  // elle devient la preuve publique du travail et s'ouvre a tout le monde.
  // Une analyse d'un match a venir ou en cours reste strictement payante.
  //
  // Cette fonction ne decide rien toute seule : elle ne fait que trancher a
  // partir d'un statut et d'une heure de coup d'envoi deja lus dans NOS
  // donnees. La decision qui fait autorite est prise COTE SERVEUR par
  // supabase/functions/match-data/index.ts, qui porte une copie fonctionnelle
  // de ce meme calcul (Deno ne charge pas le module CommonJS du site) ;
  // tests/match-page-results.test.js execute les deux et verifie qu'elles
  // decident la meme chose. Cote navigateur, elle sert uniquement a savoir
  // s'il vaut la peine d'aller POSER la question au serveur.
  //
  // Critere volontairement conservateur :
  //   1. statut de report, d'annulation, de suspension, de match arrete ou
  //      d'horaire a definir -> FERME, meme longtemps apres l'heure prevue :
  //      le match peut encore se jouer, le pari garde sa valeur ;
  //   2. sans heure de coup d'envoi exploitable -> FERME (jamais d'ouverture
  //      au jugement) ;
  //   3. sinon, ouverture 3 h 30 apres le coup d'envoi seulement. Un match
  //      dure au plus ~2 h 05 (2 h 30 avec de longs arrets) : la marge couvre
  //      les prolongations, les tirs au but et un coup d'envoi retarde ;
  //   4. un statut de fin reel (FT/AET/PEN) est enregistre comme raison mais
  //      ne dispense JAMAIS de la marge : les fichiers publics ne sont
  //      reecrits qu'aux passages du pipeline, un statut peut donc etre en
  ...
  function openedState(status, kickoffMs, nowMs) {
  ...
  // Reglement d'un match, tel que le produit le lot R1 : une ligne de
  // results/<YYYY-MM-DD>.json (cle `id`) ou de la table match_results (cle
  // `fixture_id`), contrat decrit dans docs/SPEC_RESULTATS_HIER.md.
  //
  // Rien n'est devine : la ligne doit porter le MEME identifiant que le match
  // affiche et un resultat REGLE ('win', 'loss' ou 'void'). Un match en
  // attente ('pending'), un resultat inconnu ou une ligne d'un autre match ne
  // donnent aucun bandeau plutot qu'un verdict approximatif. Un score
  // illisible est simplement absent : le verdict reste, le score n'est pas
  // invente.
  const SETTLED_OUTCOMES = ['win', 'loss', 'void'];
  const SCORE_RE = /^\s*(\d{1,2})\s*[-–]\s*(\d{1,2})\s*$/;
  // Source de la cote affichee telle qu'elle est : « (Pinnacle) » seulement
  // quand la cote vient vraiment de Pinnacle, sinon « (cotes moyennes) ».
  // Sans source connue (ni sur la ligne de reglement, ni sur le match), rien
  // n'est affiche entre parentheses - jamais une source supposee.
  function resultOddsSource(row, raw) {
    const brut = text(row && row.odds_source) || text(raw && raw.market_source);
    if (!brut) return null;
    if (/pinnacle/i.test(brut)) return 'pinnacle';
    if (/moyenne|average/i.test(brut)) return 'average';
    return null;
  }
  function matchResultView(row, matchId, raw) {
    if (!row || typeof row !== 'object') return null;
    const rowId = row.id != null ? row.id : (row.fixture_id != null ? row.fixture_id : null);
    if (rowId == null || matchId == null || String(rowId) !== String(matchId)) return null;
    const outcome = String(row.result == null ? '' : row.result).trim().toLowerCase();
    if (SETTLED_OUTCOMES.indexOf(outcome) === -1) return null;
    const score = SCORE_RE.exec(String(row.score == null ? '' : row.score));
    const odds = finite(row.cote);
    const usable = odds !== null && odds > 1;
    return {
      outcome,
      score: score ? score[1] + '-' + score[2] : null,
      pick: text(row.pick),
      marketId: text(row.market_id),
      odds: usable ? odds : null,
      oddsSource: usable ? resultOddsSource(row, raw) : null
    };
  }
  ...
  function headToHead(items) {
    const rows = (Array.isArray(items) ? items : []).map(item => ({
      date: text(item && item.d), home: teamText(item && item.home), away: teamText(item && item.away),
  ...
  function formRows(rows) {
    return (Array.isArray(rows) ? rows : []).slice(0, 5).filter(r => r && ['W', 'D', 'L'].includes(r.result)).map(r => ({
      result: r.result, score: text(r.score), opponent: teamText(r.opponent), home: r.home === true, date: text(r.d)
    }));
  }
  ...
  function buildMatchViewModel(raw) {
    ...
    return {
      ...
      // Stade : ville seulement si verifiee ; meteo seulement pour une ville
      // verifiee (elle est cherchee par ville : « Sao Paulo » pour Saint Paul
      // avant le 19/09/2026). lib/venue.js.
      conditions: {
        venue: venueLib ? venueLib.venueLabel(raw.stade) : text(raw.stade && raw.stade.nom),
        weather: display && (!venueLib || venueLib.hasVerifiedCity(raw.stade)) ? display.weatherForDisplay(raw.stade) : null
      },
  ...
  return {
    buildMatchViewModel, localizedNarrative,
    // Ouverture des analyses de matchs termines (lot R4, 20/09/2026).
    openedState, matchResultView,
    SETTLED_MARGIN_MS, FINISHED_STATUSES, BLOCKING_STATUSES
  };
});
