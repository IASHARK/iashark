...
function kickoffParis(iso) {
  // Deja au format public (heure de Paris, sans fuseau) : rendu tel quel.
  // Date.parse le lirait en heure LOCALE de la machine ; sur le runner du
  // pipeline (UTC), 18:00 devenait 20:00 (21/09/2026).
  const pub = /^(\d{4}-\d{2}-\d{2}) (\d{2}:\d{2})$/.exec(String(iso || ""));
  if (pub) return pub[1] + " " + pub[2];
