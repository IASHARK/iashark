...
function pickFreeMatch(list,horloge,market){
  ...
  if(designe.length){
    ...
    // demain). A minuit (heure de Paris) le site bascule donc tout seul sur
    // celle du nouveau jour, sans attendre le prochain passage du pipeline.
    var jourDe=function(m){ return String(m&&m.date||'').slice(0,10); };
    if(market){
      ...
      if(duMarcheDuJour.length)return duMarcheDuJour[0];
    ...
    if(designeDuJour.length)return designeDuJour[0];
