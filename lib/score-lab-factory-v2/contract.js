...
// PROSPECTIF (2026-09-06) - jamais applique retroactivement a une ligue
// dont le holdout a deja ete ouvert (Ligue1/Bundesliga/Eredivisie/LaLiga/
// J1/Serie A restent sous l'ancien contrat MONOLITHIQUE, non rejoue).
// Constat : un contrat ou UN SEUL marche degrade (ex. O/U2.5) rejette
// TOUTE la ligue (y compris l'Exact Score, deja valide independamment)
// est trop grossier pour l'objectif produit. Ce bloc separe :
//   SCORE_CORE_VALIDATED = tous les gates non-marche passent (identique
//     a l'ancien VALIDATED, MOINS MARKET_MARGINALS)
//   chaque famille de marche est evaluee INDEPENDAMMENT, seulement SI
//     SCORE_CORE_VALIDATED=YES - une famille FAIL est BLOCKED (jamais
//     utilisee pour produire un pari de CE type), une famille PASS est
//     utilisable seule.
// MARKET_TEAM_TOTALS n'a AUCUN marche calcule dans ce lab
// (candidates.js#secondaryMetricsFromPredictions ne produit que
// ou25/btts/x12) - jamais invente, toujours NOT_EVALUATED.
function buildMarketFamilyContract({ secondary }) {
  return {
    description: "Extension PROSPECTIVE (contract_version=V2.1_MARKET_FAMILY_SPLIT) : chaque famille de marche degrade INDEPENDAMMENT plutot qu'un seul gate MARKET_MARGINALS qui rejette toute la ligue sur un seul marche en echec.",
    max_relative_degradation: MAX_SECONDARY_DEGRADATION,
    source: "Meme seuil que MARKET_MARGINALS (lib/promotion.js#MAX_SECONDARY_DEGRADATION), applique par famille au lieu d'un ET global.",
    families: {
      MARKET_1X2: { metric: "x12_logloss", oos_dev_logloss: secondary.x12_logloss, pass_condition: `logloss_holdout <= ${secondary.x12_logloss} * (1+${MAX_SECONDARY_DEGRADATION})` },
      MARKET_BTTS: { metric: "btts_logloss", oos_dev_logloss: secondary.btts_logloss, pass_condition: `logloss_holdout <= ${secondary.btts_logloss} * (1+${MAX_SECONDARY_DEGRADATION})` },
      MARKET_TOTALS: { metric: "ou25_logloss", oos_dev_logloss: secondary.ou25_logloss, pass_condition: `logloss_holdout <= ${secondary.ou25_logloss} * (1+${MAX_SECONDARY_DEGRADATION})` },
      MARKET_TEAM_TOTALS: { status: "NOT_EVALUATED_NO_MARKET_COMPUTED", reason: "Aucun marche team-totals implemente dans lib/score-lab-factory-v2/candidates.js#secondaryMetricsFromPredictions - jamais invente ni approxime." },
    },
    decision_rule: {
      SCORE_CORE_VALIDATED_if: "POINT_IN_TIME_INTEGRITY, REPRODUCIBILITY, EXACT_SCORE_NLL, DATA_COVERAGE, CALIBRATION, TEMPORAL_STABILITY, NO_CATASTROPHIC_SECONDARY_DEGRADATION passent TOUS (identique a l'ancien VALIDATED, MOINS les marches).",
      per_family: "Chaque famille evaluee UNIQUEMENT si SCORE_CORE_VALIDATED=YES. FAIL => famille BLOCKED (jamais utilisee pour un pari de ce type). PASS => famille utilisable seule.",
      CANONICAL_USABLE_if: "SCORE_CORE_VALIDATED=YES, INDEPENDAMMENT du nombre de familles de marche en echec (l'Exact Score reste utilisable meme si aucun marche additionnel ne passe).",
      FULLY_VALIDATED_if: "SCORE_CORE_VALIDATED=YES ET TOUTES les familles evaluees (1X2, BTTS, TOTALS) PASS - equivalent au VALIDATED historique (verdict inchange dans ce cas).",
      never_retroactive: "Applique UNIQUEMENT aux ligues dont le holdout etait encore SEALED au moment du gel de ce contrat. Jamais rejoue sur Ligue1/Bundesliga/Eredivisie/LaLiga/J1/Serie A (contrat deja consomme sous l'ancien format monolithique, jamais reecrit).",
    },
  };
}
...
// marketFamilyProspective : true UNIQUEMENT pour une ligue dont le
// holdout est encore totalement SEALED au moment de l'appel - jamais
// pour re-geler une ligue deja consommee (voir buildMarketFamilyContract
// ci-dessus). Parametre EXPLICITE, aucune valeur par defaut implicite.
function buildProductionValidationContract({ leagueKey, championModelId, championPredictions, championParams, codeSha, datasetHashes, seedPrefix, exactScoreNLL, marketFamilyProspective }) {
  ...
  const contract = {
    ...
    decision_rule: {
      ...
      INCONCLUSIVE_if: marketFamilyProspective
        ? "DATA_COVERAGE echoue OU au moins un des gates {CALIBRATION, TEMPORAL_STABILITY, NO_CATASTROPHIC_SECONDARY_DEGRADATION} echoue sans REJECT deja declenche (MARKET_MARGINALS/familles de marche EXCLUES de ce calcul - voir market_family_contract)"
        : "DATA_COVERAGE echoue OU au moins un des gates {CALIBRATION, MARKET_MARGINALS, TEMPORAL_STABILITY, NO_CATASTROPHIC_SECONDARY_DEGRADATION} echoue sans REJECT deja declenche",
      VALIDATED_if: marketFamilyProspective ? "SCORE_CORE_VALIDATED=YES (voir market_family_contract - les marches n'affectent plus ce champ)" : "TOUS les gates PASS",
  ...
  if (marketFamilyProspective) {
    contract.contract_version = "V2.1_MARKET_FAMILY_SPLIT";
    contract.market_family_contract = buildMarketFamilyContract({ secondary });
  }
  return contract;
}
