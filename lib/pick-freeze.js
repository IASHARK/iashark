...
//
// CAS (freezeAnalysis) :
//  - garde coup d'envoi fermee (match commence, imminent, reporte PST, annule
//    CANC... : lib/kickoff-guard.js) -> rien ne change ici : le pipeline a deja
//    retire le pari (closeMatchForPick) et vide la ligne premium. Si le match
//    revient plus tard en NS, c'est une nouvelle premiere publication ;
...
//   FROZEN | FIRST_PUBLICATION | NOT_FREEZABLE (aucun pari a figer) |
...
function freezeAnalysis(fresh, previous, opts) {
  ...
  // Garde coup d'envoi : un match ferme ne retrouve jamais un pari ici.
  const gate = kickoffGate(opts.fixture, nowMs, { marginMinutes: KICKOFF_MARGIN_MINUTES });
  if (!gate.open) return result("KICKOFF_CLOSED", gate.reason, fresh, row);
