...
(function (global) {
  ...
  var TERMS_VERSION = "2026-09-16";
  // Version propre a un repertoire quand ses CGV ont change seules. 18/09/2026 :
  // CGV francaises ramenees a l'offre reellement vendue (abonnement mensuel) ;
  // les pages racine sont francaises et renvoient vers ces CGV.
  var TERMS_VERSIONS = { fr: "2026-09-18", "": "2026-09-18" };
  ...
  function buildHtml(regime, t, hrefs, uid) {
    ...
    var html = '<p class="iash-consent-title">' + esc(t("title")) + "</p>"
      ...
      + '<input type="checkbox" id="' + id + 'Terms" data-consent="terms" required aria-required="true">'
      + "<span>" + withLinks(t(regime.termsKey), t, hrefs) + ' <span class="iash-consent-req">(' + esc(t("required")) + ")</span></span></label>";
    if (regime.waiverKey) {
      html += '<label class="iash-consent-row" for="' + id + 'Waiver">'
        + '<input type="checkbox" id="' + id + 'Waiver" data-consent="waiver" required aria-required="true">'
        + "<span>" + esc(t(regime.waiverKey)) + ' <span class="iash-consent-req">(' + esc(t("required")) + ")</span></span></label>";
    }
