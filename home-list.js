...
// Mesure reelle du marche retenu, telle qu'elle arrive du flux. Forme stricte
// (une valeur inattendue est ignoree plutot qu'affichee de travers).
var OBS_KINDS=['shots','shots_on_target','goals_total','goals_home','goals_away','halftime_goals','btts','outcome'];
function cleanObserved(o){
  if(!o||typeof o!=='object')return null;
  var k=String(o.kind||'');
  if(OBS_KINDS.indexOf(k)===-1)return null;
  if(k==='btts')return o.value===true||o.value===false?{kind:k,value:o.value}:null;
  if(k==='outcome')return ['home','draw','away'].indexOf(String(o.value))!==-1?{kind:k,value:String(o.value)}:null;
  // 0 est une mesure VALIDE (« l'equipe n'a pas marque »). rnum() refuse 0
  // parce qu'il sert aux cotes ; ici on accepte tout entier positif ou nul.
  var n=parseFloat(o.value);
  return isFinite(n)&&n>=0?{kind:k,value:n}:null;
}
...
function cleanResult(e){
  ...
  return {id:String(id),home:rtxt(e.home)||'',away:rtxt(e.away)||'',
    home_id:rnum(e.home_id),away_id:rnum(e.away_id),
    ...
    observed:cleanObserved(e.observed),
...
// CE QUI S'EST PASSE sur le marche retenu (20/09/2026, demande du
// proprietaire) : « moins de 27,5 tirs -> 28 tirs dans le match ». La cote
// n'est plus affichee. Le score du match n'apparait que quand le marche porte
// vraiment sur les buts, jamais « pour decorer ».
// Mesure absente (donnee manquante) : la recommandation seule, jamais un 0.
function observedText(e){
  var o=e&&e.observed;
  if(!o||o.value==null)return '';
  var n=Number(o.value),un=n===1;
  switch(o.kind){
    case 'shots':return tf('home_list.res_obs_shots','{n} tirs dans le match',{n:n});
    case 'shots_on_target':return tf('home_list.res_obs_sot','{n} tirs cadrés',{n:n});
    case 'goals_total':return un?t('home_list.res_obs_goals_one','1 but dans le match')
      :tf('home_list.res_obs_goals','{n} buts dans le match',{n:n});
    case 'halftime_goals':return un?t('home_list.res_obs_ht_one','1 but à la mi-temps')
      :tf('home_list.res_obs_ht','{n} buts à la mi-temps',{n:n});
    case 'goals_home':return n===0?tf('home_list.res_obs_team_none','{team} n’a pas marqué',{team:e.home})
      :un?tf('home_list.res_obs_team_one','{team} : 1 but',{team:e.home})
      :tf('home_list.res_obs_team','{team} : {n} buts',{team:e.home,n:n});
    case 'goals_away':return n===0?tf('home_list.res_obs_team_none','{team} n’a pas marqué',{team:e.away})
      :un?tf('home_list.res_obs_team_one','{team} : 1 but',{team:e.away})
      :tf('home_list.res_obs_team','{team} : {n} buts',{team:e.away,n:n});
    case 'btts':return o.value===true?t('home_list.res_obs_btts_yes','les deux équipes ont marqué')
      :t('home_list.res_obs_btts_no','une équipe n’a pas marqué');
    case 'outcome':return o.value==='draw'?t('home_list.res_obs_draw','match nul')
      :tf('home_list.res_obs_win','victoire de {team}',{team:o.value==='home'?e.home:e.away});
    default:return '';
  }
}
// Recommandation publiee avant le coup d'envoi, puis la mesure reelle.
// Sans reglement, rien du tout : le match n'est pas prouve termine.
function pickHtml(e){
  if(!hasScore(e))return '';
  var obs=observedText(e);
  return '<span class="hl-rlabel">'+esc(t('home_list.res_reco','Recommandé'))+'</span>'
    +'<span class="hl-rpick">'+esc(e.pick)+'</span>'
    +(obs?'<span class="hl-robs"><span class="hl-rarrow" aria-hidden="true">→</span>'+esc(obs)+'</span>':'');
}
function pickAria(e){
  if(!hasScore(e))return '';
  var obs=observedText(e);
  return tf('home_list.res_aria_pick','Recommandé : {pick}.',{pick:e.pick})
    +(obs?' '+tf('home_list.res_aria_obs','Résultat : {obs}.',{obs:obs}):'');
}

function renderResultsBanner(list,H){
  var tot=resultTotals(list),notes=[];
  var head=!tot.settled?t('home_list.res_count_none','Résultats d’hier : aucune recommandation réglée pour le moment.')
    :tot.won<2?tf('home_list.res_count_one','Résultats d’hier : {won} recommandation réalisée sur {total}',{won:tot.won,total:tot.settled})
    :tf('home_list.res_count','Résultats d’hier : {won} recommandations réalisées sur {total}',{won:tot.won,total:tot.settled});
  ...
  if(tot.pending)notes.push(tot.pending===1?t('home_list.res_pending_one','1 recommandation en attente de règlement, hors décompte.')
    :tf('home_list.res_pending_many','{n} recommandations en attente de règlement, hors décompte.',{n:tot.pending}));
...
function resultAsMatch(e){
  return {id:e.id,league:e.league,league_key:e.league_key,
    home:{n:e.home,id:e.home_id},away:{n:e.away,id:e.away_id},
...
function renderResultsDay(list,ctx,H){
  // Journee passee : ni etoile de competition, ni etoile de match (demande du
  // proprietaire, 20/09/2026). On ne met pas en favori un jour deja joue.
  var c=Object.assign({},ctx||{},{isResultsDay:true});
  return renderResultsBanner(list,H)+renderDayBody((list||[]).map(resultAsMatch),c,H);
}
...
function matchStarHtml(m,ctx){
  ...
  // Onglet « Hier » : aucune etoile (20/09/2026).
  if(ctx&&ctx.isResultsDay)return '';
...
function renderMatchRow(m,ctx,H,index){
  ...
  // Nombre de simulations : seulement s'il est reellement connu. Sans garde,
  // un contexte sans valeur affichait « NaN simulations ».
  var sims=Number(ctx&&ctx.simulations);
  if(sig&&isFinite(sims)&&sims>0)tags.push('<span class="hl-tag hl-tag-sim">'+esc(tf('home_list.sims','{n} simulations',{n:fmtInt(sims)}))+'</span>');
    ...
    zone='<span class="hl-zone">'
      +(a.prob!=null
        ?'<span class="hl-prob-row">'
          ...
          // Pari retenu sous la probabilite (20/09/2026, demande du
          // proprietaire) : un abonne voit d'un coup d'oeil ce qui est
          // recommande sans ouvrir la page. Affiche seulement quand l'analyse
          // est deja ouverte pour ce visiteur (abonne, ou analyse offerte) :
          // analysisFor n'a lu pari_rec que dans ce cas.
          +(a.market?'<span class="hl-openpick"><span class="hl-openpick-l">'+esc(t('home_list.reco_short','Pari retenu'))+'</span>'
            +'<span class="hl-openpick-v">'+esc(a.market)+'</span></span>':'')
  ...
  var anaAria=a.state==='result'?[pickAria(a.result),verdictLabel(resultOf(a.result))+'.'].filter(Boolean).join(' ')
    ...
    :a.state==='open'?(a.prob!=null?tf('home_list.aria_prob','Probabilité estimée {p} sur 10.',{p:a.prob}):t('home_list.aria_open','Analyse disponible.'))
      +(a.market?' '+tf('home_list.aria_reco','Pari retenu : {market}.',{market:a.market}):'')
...
function renderLeagueBlock(g,ctx,H,startIndex){
  ...
  return '<section class="hl-league'+(fav?' is-fav':'')+(open?'':' is-collapsed')+'" data-league="'+k+'">'
    ...
    // Onglet « Hier » : pas d'etoile de favori (demande du proprietaire,
    // 20/09/2026). Une journee passee ne se suit pas, elle se constate.
    +(ctx.isResultsDay?'':'<button type="button" class="hl-star'+(fav?' is-on':'')+'" data-hl-fav="'+k+'" aria-pressed="'+fav+'" aria-label="'+esc(favLbl)+'" title="'+esc(favLbl)+'"'+track+'>'+ICON.star+'</button>')
...
function mount(rootEl,options){
  ...
  function renderBody(animate){
    ...
    // Jour affiche = la veille : l'onglet « Hier » n'affiche pas d'etoile de
    // favori (demande du proprietaire, 20/09/2026).
    ctx.isResultsDay=res;
