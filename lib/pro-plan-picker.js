...
(function (global) {
  ...
  function textFor(key, market, labels) {
    ...
    if (I && typeof I.t === "function" && I.dict) {
      var v = code ? I.t("pro_plans." + key + "_" + code, null) : null;
      if (typeof v === "string" && v) return v;
      v = I.t("pro_plans." + key, null);
      if (typeof v === "string" && v) return v;
    }
    return FALLBACK[key];
  }
  ...
  function pickDefault(offer, current) {
    var sold = offer.intervals.filter(function (i) { return i.amount != null; }).map(function (i) { return i.interval; });
    if (current && sold.indexOf(current) !== -1) return current;
    if (sold.indexOf(offer.defaultInterval) !== -1) return offer.defaultInterval;
    return sold[0] || null;
  }

  // HTML du selecteur. t(key) = texte ; availability = {iv: false} pour
  // marquer une duree "bientot disponible" (secret Stripe absent).
  function buildHtml(offer, t, uid, selected, availability) {
    ...
    offer.intervals.forEach(function (it) {
      ...
      var iv = it.interval, id = name + "_" + iv, soon = availability[iv] === false;
      html += '<label class="iash-plan' + (iv === selected ? " is-selected" : "") + (soon ? " is-soon" : "") + '" for="' + id + '" data-interval="' + iv + '">'
  ...
  function mount(container, options) {
    ...
    var api = {
      ...
      isAvailable: function (iv) { return availability[iv || selected] !== false; },
