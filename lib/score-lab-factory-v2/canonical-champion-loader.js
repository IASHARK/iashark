...
function loadCanonicalScoreChampion(leagueKey) {
  ...
  // PROSPECTIF (voir contract.js#buildMarketFamilyContract) : expose la
  // ventilation par famille de marche quand elle existe, pour qu'un
  // futur appelant ne produise JAMAIS un pari sur une famille BLOCKED -
  // absent pour toute ligue gelee sous l'ancien contrat monolithique.
  if (report.market_family_status) {
    result.market_family_status = report.market_family_status;
    result.canonical_usable = report.canonical_usable;
    result.fully_validated = report.fully_validated;
  }
  return result;
}
