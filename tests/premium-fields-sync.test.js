...
// affichage Pro. Audit fuite du 14/09/2026 (hotfix main).
...
test("pipeline : liste unique, premium_fields persistes, data.json et historique assainis", () => {
...
test("page Outils : donnees de match seulement via match-data pour un abonne", () => {
  ...
  assert.doesNotMatch(js, /fetch\(['"`]\/?data\.json/, "la page Outils ne lit jamais le fichier public de matchs");
});
...
test("accueils : ne demandent plus historique.json", () => {
  for (const f of ["index.html", "fr/index.html", "gb/index.html", "mx/index.html"]) {
...
test("migration : colonne premium_fields", () => {
