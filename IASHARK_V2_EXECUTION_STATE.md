...
| AUTOMATED_TESTS_PASS | EN COURS | voir section Tests |
...
- Vraie exécution CI (le fichier de workflow de tests sera créé et correct, mais je ne peux pas observer un run GitHub Actions réel depuis cette session — seulement `node --check`/exécution locale des tests).
