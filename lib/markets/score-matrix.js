...
function deriveMarketsFromMatrix(matrix) {
  ...
  // Marches combines (resultat + total buts, resultat + BTTS) : joints
  // REELS calcules cellule par cellule sur la meme matrice, jamais une
  // multiplication naive des deux probabilites marginales (p1 * pOver25
  // serait faux - un match ou le domicile gagne large a structurellement
  // plus de buts qu'un match nul serre, les deux evenements ne sont pas
  // independants). Seule la ligne 2.5 est retenue pour le combine
  // resultat+total (la plus liquide/observee reellement, cf. bet_id 25
  // "Result/Total Goals" - pas besoin des 7 lignes pour ce marche precis).
  let p1Over25 = 0, p1Under25 = 0, pNOver25 = 0, pNUnder25 = 0, p2Over25 = 0, p2Under25 = 0;
  let p1Btts = 0, p1NoBtts = 0, pNBtts = 0, pNNoBtts = 0, p2Btts = 0, p2NoBtts = 0;
  ...
  for (let h = 0; h <= maxH; h++) {
    for (let a = 0; a <= maxA; a++) {
      ...
      let side; // '1' | 'N' | '2'
      if (h > a) { p1 += p; side = "1"; } else if (h === a) { pN += p; side = "N"; } else { p2 += p; side = "2"; }
      if (isBtts) bttsYes += p; else bttsNo += p;
      ...
      if (side === "1") { if (over25) p1Over25 += p; else p1Under25 += p; if (isBtts) p1Btts += p; else p1NoBtts += p; }
      else if (side === "N") { if (over25) pNOver25 += p; else pNUnder25 += p; if (isBtts) pNBtts += p; else pNNoBtts += p; }
      else { if (over25) p2Over25 += p; else p2Under25 += p; if (isBtts) p2Btts += p; else p2NoBtts += p; }
    }
  }
  ...
  return {
    ...
    goalBands: bands,
    resultAndOver25: { home: p1Over25, draw: pNOver25, away: p2Over25 },
    resultAndUnder25: { home: p1Under25, draw: pNUnder25, away: p2Under25 },
    resultAndBtts: { home: p1Btts, draw: pNBtts, away: p2Btts },
    resultAndNoBtts: { home: p1NoBtts, draw: pNNoBtts, away: p2NoBtts },
  };
}
