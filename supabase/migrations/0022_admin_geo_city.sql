-- IASHARK - Tableau de bord admin : villes et regions approximatives.
--
-- NON APPLIQUEE. A relire par le lead, puis appliquer APRES
-- 0019_admin_dashboard_v2.sql (deja en production) ET apres le deploiement de
-- l'Edge Function /api/geo (netlify/edge-functions/geo.ts) qui fait remonter
-- geo_country / geo_region / geo_city dans funnel_events.metadata.
-- Le tableau de bord (admin.html) fonctionne avec OU sans cette migration :
-- sans elle, il affiche seulement les pays estimes.
--
-- Contenu : les trois fonctions admin de lecture ci-dessous sont remplacees
-- (create or replace, MEMES signatures et meme type de retour que 0019,
-- memes controles admin_is_admin(), SECURITY DEFINER, lecture seule) :
-- - admin_analytics : + 'cities' (30 premieres villes : pays, region, ville,
--   visiteurs, inscrits), + 'geo' (cities_available, visiteurs avec ville),
--   + 'internal_reasons' (visites retirees des chiffres par raison : bot,
--   emulated, internal, qa, excluded, account) ; chaque visite interne porte
--   sa raison ;
-- - admin_live_view : chaque visiteur actif porte region, city, tz et
--   internal_reason ; + 'geo_available' ;
-- - admin_recent_sessions : chaque visite porte region, city et tz.
-- Pays : geo_country (reseau) s'il est valide, sinon country_guess (fuseau).
-- Fuseau (tz) : deja collecte depuis 0008, valide par regex avant d'etre
-- renvoye ; sert a afficher l'heure locale du visiteur.
-- Aucune adresse IP n'existe dans funnel_events : rien a lire, rien a stocker.
-- Toutes les valeurs texte lues dans metadata (ecrit par anon) sont bornees
-- (left(..., 60)) et le tableau de bord les echappe avant affichage.
...
begin
  ...
  return (
    ...
    -- Une ligne par visite interne, avec sa raison : la premiere raison
    -- explicite trouvee dans metadata, a defaut 'qa' (session_id qa...) ou
    -- 'account' (compte admin ou de test).
    flagged as (
      select distinct on (e.session_id) e.session_id,
        coalesce(public.admin_internal_reason(e.metadata),
          case when e.session_id ilike 'qa%' then 'qa' else 'account' end) as reason
      from public.funnel_events e, params p
      where e.session_id is not null
      ...
      order by e.session_id, (public.admin_internal_reason(e.metadata) is null), e.created_at
    ),
    sess as (
    ...
    sess_dim as (
      select s.*,
        ...
        left(coalesce(case when fp.md->>'geo_country' ~ '^[A-Z]{2}$' then fp.md->>'geo_country' end, fp.md->>'country_guess'), 8) as country,
        left(nullif(btrim(fp.md->>'geo_region'), ''), 60) as region,
        left(nullif(btrim(fp.md->>'geo_city'), ''), 60) as city,
        case when fp.md->>'tz' ~ '^[A-Za-z_]+(/[A-Za-z0-9_+-]+){0,2}$' then left(fp.md->>'tz', 48) end as tz,
    ...
    -- Villes approximatives (reseau, via /api/geo) : 30 premieres.
    cities as (
      select country, region, city, count(*) as visitors, count(*) filter (where signed_up) as signups
      from cur_sess where city is not null
      group by 1, 2, 3 order by visitors desc, city limit 30
    ),
    sites as (
    ...
    select json_build_object(
      ...
      'cities', (select coalesce(json_agg(t order by t.visitors desc, t.city), '[]'::json) from cities t),
      'geo', json_build_object(
        'cities_available', true,
        'visitors', (select count(*) from cur_sess),
        'visitors_with_city', (select count(*) from cur_sess where city is not null)
      ),
      'internal_reasons', (select coalesce(json_agg(t order by t.visitors desc, t.reason), '[]'::json)
        from (select coalesce(internal_reason, 'account') as reason, count(*) as visitors
              from sess_dim where period = 'cur' and is_internal group by 1) t),
...
begin
  ...
  return (
    ...
    dim as (
      ...
      left join lateral (
        select true as flag,
          coalesce(public.admin_internal_reason(x.metadata),
            case when x.session_id ilike 'qa%' then 'qa' else 'account' end) as reason
        from public.funnel_events x
        ...
        order by (public.admin_internal_reason(x.metadata) is null), x.created_at
        limit 1
      ) fl on true
      left join lateral (
    ...
    enriched as (
      select d.*,
        ...
        left(coalesce(case when d.first_md->>'geo_country' ~ '^[A-Z]{2}$' then d.first_md->>'geo_country' end, d.first_md->>'country_guess'), 8) as country,
        left(nullif(btrim(d.first_md->>'geo_region'), ''), 60) as region,
        left(nullif(btrim(d.first_md->>'geo_city'), ''), 60) as city,
        case when d.first_md->>'tz' ~ '^[A-Za-z_]+(/[A-Za-z0-9_+-]+){0,2}$' then left(d.first_md->>'tz', 48) end as tz,
    ...
    select json_build_object(
      ...
      'visitors', (select coalesce(json_agg(json_build_object(
          ...
          'internal_reason', a.internal_reason, 'region', a.region, 'city', a.city, 'tz', a.tz
...
begin
  ...
  return (
    ...
    select coalesce(json_agg(json_build_object(
        ...
        'region', d.region, 'city', d.city, 'tz', d.tz,
