...
//   - MIN_RELATIVE_GAIN=0.0025 (0.25%) : seuil de promotion PRE-ENREGISTRE
//     dans la SPEC M1 avant toute experience (scripts/experiments/exp001_manifest.json
//     #methodology.promotion_thresholds) - corrige le 2026-09-05 : ce
//     module utilisait par erreur 0.5% ("plancher economique"), une valeur
//     jamais enregistree dans la SPEC. A ce stade (EXP-001) on evalue la
//     qualite PROBABILISTE du modele (le NLL diminue-t-il assez pour que
//     ce ne soit pas du bruit d'optimisation), pas la rentabilite d'un
//     pari - d'ou le renommage GAIN_BELOW_ECONOMIC_FLOOR ->
//     GAIN_BELOW_PROMOTION_THRESHOLD (aucune connotation "betting" ici).
...
const MIN_RELATIVE_GAIN = 0.0025;
...
//     BOOTSTRAP_CI_CROSSES_ZERO ne se declenchent, les trois cas etant
//     exhaustifs et mutuellement exclusifs pour un intervalle ci_lower<=ci_upper).
...
function evaluatePromotion(input) {
  ...
  details.promotion_threshold = MIN_RELATIVE_GAIN;
  if (relativeGain < MIN_RELATIVE_GAIN) {
    rejectReasons.push(REASON.GAIN_BELOW_PROMOTION_THRESHOLD);
  }
  ...
  const ciCrossesZero = input.ci_lower <= 0 && input.ci_upper >= 0;
  if (ciCrossesZero) {
    shadowReasons.push(REASON.BOOTSTRAP_CI_CROSSES_ZERO);
    details.ci_lower = input.ci_lower;
    details.ci_upper = input.ci_upper;
  }

  if (rejectReasons.length) {
    // Diagnostic supplementaire, meme quand REJECT est declenche par un
    // AUTRE motif (ex: gain sous le seuil) : signale explicitement que
    // l'IC bootstrap ne permettait de toute facon pas de conclure
    // statistiquement - jamais laisser croire qu'un resultat "juste sous
    // le seuil" aurait ete tranche par un seuil legerement different.
    const combined = ciCrossesZero && !rejectReasons.includes(REASON.BOOTSTRAP_CI_CROSSES_ZERO)
      ? [...rejectReasons, REASON.BOOTSTRAP_CI_CROSSES_ZERO]
      : rejectReasons;
    return { status: STATUS.REJECT, reason_codes: combined, details };
  }
...
module.exports = { evaluatePromotion, REASON, STATUS, MIN_N_OOS, MIN_CONVERGENCE_RATE, MAX_BOUNDARY_HIT_RATE, MIN_RELATIVE_GAIN, MAX_SECONDARY_DEGRADATION, MAX_RHO_STD, MIN_LOW_SCORE_COUNT_FOR_CHECK, MAX_LOW_SCORE_RELATIVE_DEGRADATION };
