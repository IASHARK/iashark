...
function buildDayFile(input) {
  ...
  paris.forEach(function (pari) {
    ...
    const entree = {
      ...
      score: termine ? (regle.score || null) : null,
      pick: termine ? (pari.prediction || null) : null,
      market_id: termine ? (pari.market_id || null) : null,
      cote: termine ? pari.cote : null,
      odds_source: termine ? oddsSource(pari) : null,
      result: regle.result,
    };
    // Mesure reelle du marche retenu (« 28 tirs », « match nul »...), seulement
    // sur un match termine, comme le pari lui-meme.
    const mesure = termine ? observedOf(pari, fixture, contexts[cle]) : null;
    if (mesure) entree.observed = mesure;
...
function matchResultRow(entree, day, resolvedAt) {
  ...
  if (entree.result === "pending" && entree.score === null && entree.pick === null) return null;
  return {
