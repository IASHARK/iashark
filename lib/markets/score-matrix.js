...
// GATE A4 (SPEC LAB PRO v1.0 §22) : troncature adaptative. maxGoals=10
// fixe perdait jusqu'a 1.11e-4 de masse de probabilite sur les lambdas
// reels de production (mesure le 2026-09-04, cf. audit) - six ordres de
// grandeur au-dessus du seuil <1e-10 exige. Augmente maxGoals par pas
// deterministes jusqu'a ce que la masse residuelle (1 - somme de la
// matrice AVANT renormalisation) passe sous le seuil, avec un minimum
// raisonnable (ne redescend jamais sous l'ancienne valeur fixe) et un
// garde-fou dur qui echoue explicitement plutot que de boucler
// silencieusement sur des lambdas aberrants.
const ADAPTIVE_MIN_GOALS = 10; // jamais moins que l'ancienne troncature fixe
const ADAPTIVE_STEP = 5; // croissance deterministe, pas aleatoire
const ADAPTIVE_SAFETY_CAP = 60; // tres au-dessus de tout besoin reel (bornes lambda 0.80-3.40) - protection uniquement
const ADAPTIVE_TAIL_MASS_THRESHOLD = 1e-10;

function tailMass(lambdaH, lambdaA, maxGoals) {
  let sum = 0;
  for (let h = 0; h <= maxGoals; h++) {
    for (let a = 0; a <= maxGoals; a++) {
      sum += poissonProb(lambdaH, h) * poissonProb(lambdaA, a) * dixonColesCorr(h, a, lambdaH, lambdaA);
    }
  }
  return 1 - sum;
}

// Retourne { matrix, maxGoal, tailMass } - matrix NON renormalisee (la
// renormalisation reste la responsabilite de blendMatrices en aval, comme
// avant cette experience, pour ne changer qu'une seule chose a la fois).
function buildAdaptiveDixonColesMatrix(lambdaH, lambdaA, options) {
  options = options || {};
  const threshold = options.tailMassThreshold || ADAPTIVE_TAIL_MASS_THRESHOLD;
  let maxGoals = options.minGoals || ADAPTIVE_MIN_GOALS;
  let mass = tailMass(lambdaH, lambdaA, maxGoals);
  while (mass >= threshold) {
    maxGoals += ADAPTIVE_STEP;
    if (maxGoals > ADAPTIVE_SAFETY_CAP) {
      throw new Error(
        "buildAdaptiveDixonColesMatrix: safety cap (" + ADAPTIVE_SAFETY_CAP + " buts) atteint sans convergence sous " +
        threshold + " pour lambdaH=" + lambdaH + " lambdaA=" + lambdaA + " (derniere masse residuelle=" + mass + ") - lambdas hors domaine attendu, a investiguer avant d'augmenter le cap."
      );
    }
    mass = tailMass(lambdaH, lambdaA, maxGoals);
  }
  return { matrix: buildDixonColesMatrix(lambdaH, lambdaA, maxGoals), maxGoal: maxGoals, tailMass: mass };
}
...
module.exports = { buildPoissonMatrix, buildDixonColesMatrix, blendMatrices, deriveMarketsFromMatrix, buildAdaptiveDixonColesMatrix, tailMass };
