...
test("buildDayFile : REFUSE de publier la moindre donnee de pari d'un match non termine", () => {
  ...
  for (const k of ["pick", "cote", "market_id", "odds_source", "score"]) {
    assert.equal(m[k], undefined, "champ " + k + " publie");
  }
...
test("buildDayFile : un match termine publie SON VERDICT et rien du pari", () => {
  const out = MR.buildDayFile({
    day: "2026-09-19",
  ...
  // Le fichier public ne porte QUE le verdict : ni pari, ni cote, ni source,
  // ni score. L'onglet « Hier » n'affiche qu'un liseré de couleur.
  for (const k of ["pick", "market_id", "cote", "odds_source", "score", "observed"]) {
    assert.equal(m[k], undefined, "champ " + k + " publie alors qu'il ne doit jamais l'etre");
  }
...
test("buildDayFile : une ligne match_results deja reglee fait autorite (aucun nouvel appel API)", () => {
  ...
  // Le verdict vient de la ligne deja reglee ; ni score ni pari ne sont publies.
  assert.equal(m.score, undefined);
  assert.equal(m.pick, undefined);
...
test("buildDayFile : pari masque relu depuis predictions_archive", () => {
  ...
  assert.equal(sansArchive.file.matches[0].pick, undefined);
  ...
  // Le pari relu sert a REGLER, il n'est jamais publie : seul le verdict sort.
  assert.equal(m.pick, undefined);
  assert.equal(m.cote, undefined);
  assert.equal(m.odds_source, undefined);
  assert.equal(m.result, "win");
...
test("buildDayFile : href seulement quand la page match existe vraiment", { skip: typeof require("../lib/league-names.js").staticMatchPath !== "function" && "pages match statiques absentes de cette version" }, () => {
...
test("matchResultRow : ligne complete pour un match termine, refusee pour un match en cours", () => {
  ...
  assert.deepEqual(row, {
    ...
    // La ligne de table reprend ce que porte l'entree : depuis le 20/09/2026
    // le fichier public n'a plus ni score, ni pari, ni cote.
    score: null, pick: null, market_id: null, cote: null,
    odds_source: null, result: "win",
