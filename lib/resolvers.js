...
// BLOCKER_IMPLEMENTATION corrige le 2026-09-04 : la resolution production
// (update-data.yml) lisait fix.goals.{home,away} pour regler 1X2/O-U/BTTS -
// ce champ API-Football est le score FINAL, qui inclut la prolongation
// (AET) pour un match qui y est alle. Un match 1-1 a 90 min termine 2-1
// apres prolongation etait donc regle comme une victoire domicile/Over/BTTS
// au lieu d'un nul/Under/BTTS reglementaire (aucun marche 1X2/O-U/BTTS
// publie par IASHARK n'est un marche "apres prolongation").
//
// Meme logique que lib/data/fixtures-normalizer.js#normalizeFixture (deja
// corrige cote laboratoire GATE B3) : score.fulltime.* est le score
// reglementaire 90 min, distinct de goals.* pour AET/PEN. Repli sur
// goals.* UNIQUEMENT quand le statut est FT simple (fulltime et goals
// coincident alors par definition - aucune fabrication, juste une
// tolerance si l'API omet exceptionnellement score.fulltime sur un FT).
function extractRegulationScore(fix) {
  const ft = fix && fix.score && fix.score.fulltime;
  let gh = ft && ft.home != null ? ft.home : null;
  let ga = ft && ft.away != null ? ft.away : null;
  const shortStatus = (fix && fix.fixture && fix.fixture.status && fix.fixture.status.short) || "";
  if ((gh == null || ga == null) && shortStatus === "FT") {
    gh = fix.goals && fix.goals.home != null ? fix.goals.home : null;
    ga = fix.goals && fix.goals.away != null ? fix.goals.away : null;
  }
  return { gh, ga };
}

module.exports = { resolveMarketWin, classifyFixtureStatus, extractRegulationScore, VOID_STATUSES, FINISHED_STATUSES };
