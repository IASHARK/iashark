...
//      equipes, stade, is_free). Aucun champ premium (pari_rec, cote_rec, edge,
//      conf...) n'est jamais affiche sur ces pages, meme pour le match offert.
const fs = require("fs");
    ...
    return {
      ...
      rows: g.filter(function (r) { return r && r.team; }).map(function (r) {
        ...
        return {
          ...
          // Forme (plus recent en premier, W/D/L) et zone de classement : donnees publiques api-football.
          form: typeof r.form === "string" ? r.form.toUpperCase().replace(/[^WDL]/g, "").slice(0, 5) || null : null,
