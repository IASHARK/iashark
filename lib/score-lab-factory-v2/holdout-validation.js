...
function evaluateHoldout({ contract, predictions, totalFixturesInHoldoutSeason, holdoutFixtureIds, exactScoreNLL }) {
  ...
  // PROSPECTIF (contract.contract_version === "V2.1_MARKET_FAMILY_SPLIT",
  // jamais present sur un contrat deja gele avant cette extension) :
  // MARKET_MARGINALS n'entre plus dans inconclusive_triggers/verdict -
  // chaque famille est evaluee a part dans market_family_status,
  // independamment du verdict SCORE_CORE. Jamais applique a un contrat
  // sans ce flag (branche legacy inchangee ci-dessous).
  const marketFamilyProspective = contract.contract_version === "V2.1_MARKET_FAMILY_SPLIT";
  ...
  if (!marketFamilyProspective && !marketMarginalsPass) inconclusiveTriggers.push("MARKET_MARGINALS");
  ...
  if (marketFamilyProspective) {
    const scoreCoreValidated = verdict === "VALIDATED";
    result.score_core_validated = scoreCoreValidated;
    result.market_family_status = {
      MARKET_1X2_STATUS: scoreCoreValidated ? (marketMarginals.x12.pass ? "PASS" : "BLOCKED") : "NOT_EVALUATED_SCORE_CORE_NOT_VALIDATED",
      MARKET_BTTS_STATUS: scoreCoreValidated ? (marketMarginals.btts.pass ? "PASS" : "BLOCKED") : "NOT_EVALUATED_SCORE_CORE_NOT_VALIDATED",
      MARKET_TOTALS_STATUS: scoreCoreValidated ? (marketMarginals.ou25.pass ? "PASS" : "BLOCKED") : "NOT_EVALUATED_SCORE_CORE_NOT_VALIDATED",
      MARKET_TEAM_TOTALS_STATUS: "NOT_EVALUATED_NO_MARKET_COMPUTED",
    };
    result.canonical_usable = scoreCoreValidated;
    result.fully_validated = scoreCoreValidated && marketMarginals.x12.pass && marketMarginals.btts.pass && marketMarginals.ou25.pass;
  }
