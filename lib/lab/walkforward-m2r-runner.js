...
const CHAMPION_RHO = -0.0845; // defaut PL uniquement - voir options.championRho ci-dessous
...
//   championRho (optionnel, defaut -0.0845/PL) - LEAGUE_EXPANSION_FACTORY_V1 (2026-09-06) :
//     promu en parametre pour permettre a une autre ligue d'utiliser SON
//     PROPRE rho fige (jamais rho_PL injecte automatiquement). Le defaut
//     -0.0845 est conserve UNIQUEMENT pour compatibilite retroactive des
//     appelants PL existants (scripts/run_exp002r.js, tests/lab-m2r-runner.test.js)
//     qui ne passent pas ce champ - comportement 100% inchange pour eux.
// }
function runWalkForwardM2R(options) {
  ...
  const championRho = options.championRho !== undefined ? options.championRho : CHAMPION_RHO;
  ...
  for (const { cutoff, batch } of cutoffs) {
    ...
    for (const f of batch) {
      ...
      const m0 = predictWithRho(lambdasM0.lambdaH, lambdasM0.lambdaA, championRho);
      const m2 = predictWithRho(lambdasM2.lambdaH, lambdasM2.lambdaA, championRho);
