...
function projectedSide(vm,key){const team=vm.identity[key],rows=vm.players.analytics.projected[key];if(!rows.length)return '';return `<div class="used-players"><h3>${logo(team.logo,team.name)}${esc(team.name)}</h3>${rows.map(p=>`<div><span>${p.number!==null?`#${fmt(p.number)} `:''}${esc(p.name)}</span><small>${esc(p.position||'N/D')}</small><b>${p.startProbability!==null?`${p.startProbability}%`:`${fmt(p.minutesRecent)}′`}</b></div>`).join('')}</div>`}
function lineupV3(vm){if(vm.players.lineupMode==='OFFICIAL')return `<div class="lineup-title">COMPOSITION OFFICIELLE ✓</div>${lineups(vm)}`;if(vm.players.lineupMode==='PROJECTED'){const enough=vm.players.analytics.home.fixtureCount>=5&&vm.players.analytics.away.fixtureCount>=5;return `<div class="lineup-title">${enough?'Composition projetée IASHARK':'Joueurs les plus utilisés'}</div><div class="projected-grid">${projectedSide(vm,'home')}${projectedSide(vm,'away')}</div>`}return ''}
...
function playersV3(vm){const blocks=[keyPlayersV3(vm)];const lineup=lineupV3(vm);if(lineup)blocks.push(panel(vm.players.lineupMode==='OFFICIAL'?'Compositions officielles':'Composition projetée IASHARK',lineup,'player-section'));const form=playerFormV3(vm);if(form)blocks.push(form);const threats=threatsV3(vm);if(threats)blocks.push(threats);if(vm.players.projections.length)blocks.push(panel('Props joueurs IASHARK',propsSection(vm),'player-section'));blocks.push(availabilityV3(vm));return blocks.join('')}
