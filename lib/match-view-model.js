(function (root, factory) {
  ...
  // Temps de jeu minimum pour etre propose comme buteur a surveiller :
  // l'equivalent de 3 matchs complets sur les 10 dernieres rencontres.
  const THREAT_MIN_MINUTES = 270;
  // Constante d'attenuation. Un ratio "par 90 minutes" mesure sur peu de
  // temps de jeu est un artefact : un remplacant qui marque une fois en
  // 30 minutes affiche 3,0 buts/90. On rapproche donc chaque ratio de zero
  // d'autant plus fort que l'echantillon est petit (m/(m+K)) : a 900
  // minutes le joueur garde ~77% de son ratio, a 90 minutes seulement 25%.
  const THREAT_SHRINK = 270;

  // Classement "menace de but" : qui est le plus susceptible de marquer.
  // AVANT, le tri se faisait sur les ratios bruts avec pour seul garde-fou
  // "au moins 3 apparitions" - sans aucune exigence de temps de jeu. En
  // conditions reelles, cela remontait systematiquement des joueurs ayant a
  // peine joue (un cas observe : 2,9 buts/90 calcules sur 31 minutes et
  // aucune titularisation), qui passaient devant des titulaires reguliers.
  // Desormais : un plancher de temps de jeu, et un score attenue selon la
  // taille de l'echantillon. Les paliers de repli garantissent qu'on ne
  // renvoie jamais une liste vide sur un championnat peu couvert - le
  // drapeau thinSample signale alors a l'affichage que l'echantillon est
  // faible, au lieu de le taire.
  function scoringThreatRanking(allPlayers) {
    const eligible = allPlayers.filter(p =>
      p.appearances >= 3 && !p.absent && ((p.goals90 || 0) > 0 || (p.shotsOn90 || 0) > 0));
    const scoreOf = p => {
      const m = finite(p.minutes) || 0;
      const weight = m / (m + THREAT_SHRINK);
      return Math.round(((p.goals90 || 0) * 1.5 + (p.shotsOn90 || 0) * 0.6) * weight * 100) / 100;
    };
    for (const floor of [THREAT_MIN_MINUTES, 180, 90, 0]) {
      const pool = eligible.filter(p => (finite(p.minutes) || 0) >= floor);
      if (pool.length) {
        return pool
          .map(p => ({ ...p, threatScore: scoreOf(p), thinSample: floor < THREAT_MIN_MINUTES }))
    ...
    const side = (key, teamValue) => {
      ...
      const players = Object.keys(grouped).map(id => {
        ...
        return {
          ...
          ratings: ratings.map(round1), minutes, minutesRecent, appearances, starts, startProbability, absent
        };
  ...
  function withScoringProbability(list) {
    return (list || []).map(p => {
      const expectedMinutes = p.startProbability !== null ? Math.round((p.startProbability / 100) * 90) : (p.minutesRecent && p.appearances ? Math.round(p.minutesRecent / p.appearances) : null);
