...
test("la page simple expose une seule colonne de sections réelles, sans onglets",()=>{
  ...
  for(const value of ['Recommandation IASHARK','Probabilités 1X2','Buts attendus','Pourquoi le pari ressort','Scores probables','Lecture du match','Scénario probable du match','Buteur à surveiller','Absents & incertains'])assert.match(js,new RegExp(value));
});
...
test("aucune section ne prétend avoir une donnée absente : chaque bloc a un état vide honnête",()=>{
  for(const value of ['Aucun marché ne franchit les seuils','Probabilités indisponibles','xG indisponibles','Statistiques comparatives indisponibles','Scores probables indisponibles','Lecture du match indisponible','Scénario du match indisponible'])assert.match(js,new RegExp(value.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
});
