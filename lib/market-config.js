/* IASHARK — configuration marche au runtime (devise, prix, aide jeu
   ...
   - /en/ : marche "us" (USD) une fois config/markets.json#_usdSwitch applique,
     marche EUR "fr" avant (_dirs.en.market decide, jamais ce fichier).
   ...
   priceIntlLocale, currency, prices, helpline, legal, checkoutMarket, status,
   formatPrice(), proOffer(), apply()}. priceIntlLocale = locale Intl des prix :
   celle du marche si config/markets.json#<marche>.priceIntlLocale existe (USD
   de /en/ : "$19.99" en en-US, jamais "US$19.99"), sinon intlLocale.
   ...
   deja pret) :
     ...
     [data-market-open-if="pro|pro.<duree>"] -> element masque (hidden) si
       l'offre (n'importe quelle duree pour "pro") n'est pas PAYABLE en ligne
       (config/markets.json#checkoutOpen) ; [data-market-closed-if] : l'inverse
       (ligne « paiement pas encore ouvert » d'un marche ferme, 19/09/2026).
     [data-market-currency] -> code devise (EUR, GBP, ZAR, MXN, USD).
...
(function (global) {
  ...
  // Offre payable en ligne ? "pro" = au moins une duree vendue ET ouverte au
  // paiement (checkoutOpen ; absent = toutes ouvertes) ; "pro.<duree>" = cette
  // duree. Fonction pure, partagee avec scripts/build-locales.js (HTML genere).
  function isPayable(prices, key, checkoutOpen) {
    var k = String(key || "");
    var m = k.match(/^pro(?:[._](week|month|year))?$/);
    if (!m) return false;
    var openList = Array.isArray(checkoutOpen) ? checkoutOpen : null;
    var ivs = m[1] ? [m[1]] : PRO_INTERVALS;
    return ivs.some(function (iv) { return proAmount(prices, iv) != null && (!openList || openList.indexOf(iv) !== -1); });
  }
  ...
  function build(dir, marketHint) {
    ...
    // Prix : locale propre au marche (priceIntlLocale), sinon celle du repertoire.
    market.priceIntlLocale = mk.priceIntlLocale || market.intlLocale;
    ...
    market.formatPrice = function (planKey) {
      ...
      return amount == null ? null : formatAmount(amount, market.currency, market.priceIntlLocale);
    };
    market.proOffer = function () { return proOffer(market.prices, market.currency, market.priceIntlLocale, market.checkoutOpen); };
    market.isPayable = function (key) { return isPayable(market.prices, key, market.checkoutOpen); };
    ...
    market.apply = function (root) {
      ...
      each(root, "[data-market-open-if]", function (el) { el.hidden = !market.isPayable(el.getAttribute("data-market-open-if")); });
      each(root, "[data-market-closed-if]", function (el) { el.hidden = market.isPayable(el.getAttribute("data-market-closed-if")); });
  ...
  global.IasharkMarketConfig = {
    ...
    priceFor: priceFor, proAmount: proAmount, proOffer: proOffer, isPayable: isPayable,
