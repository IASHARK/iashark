...
(see ../references/known-issues.md for the human-readable bug/history behind
each constant, and SKILL.md's source-of-truth table for which doc owns
which kind of information). Do not change a constant here without updating
known-issues.md too.
...
# Corrected 2026-08-23: the original (15, 40) was set before any script had
# actually been generated; the first real scripts were deliberately short
# (~8-12s) A/B test material, not a real-content target. Real videos run
# 14-50s real seconds, sized to content — see script-writing-protocol.md's
# duration-tiering rule for the three sub-ranges within this.
SCRIPT_DURATION_TARGET_SECONDS = (14, 50)
...
def check_balance_hard_stop_if_insufficient():
    """The pre-flight balance gate for full-video: ALWAYS a hard stop on
    insufficient balance, never a mere warning — never spend past what the
    account can cover, after running out mid-batch repeatedly the night this
    pipeline was built. Named separately from check_balance_sufficient's
    generic warn_only=True path (which the `estimate-cost` CLI command uses
    for a genuine non-fatal warning) so the intent is visible from the call
    site itself, not left implicit in a `warn_only=True` argument that reads
    as non-fatal but isn't, once the caller (run_full_video's record())
    turns any ok=False into sys.exit regardless of the flag."""
    return check_balance_sufficient(estimate_video_cost(), warn_only=True)
...
def generate_video_with_qc(image_url, audio_local_path, out_path, prompt="", resolution=None,
                            max_seed_attempts=3, chunk_dir=None, seed=-1):
    ...
    `seed` is used for the FIRST attempt only (-1, the default, lets the API
    pick). A caption-hallucination retry always randomizes regardless of what
    was requested — the whole point of retrying is to get something
    different from the attempt that just failed, so reusing the same
    requested seed there would defeat the retry."""
    import random
    last_ocr = None
    for attempt in range(1, max_seed_attempts + 1):
        attempt_seed = seed if attempt == 1 else random.randint(1, 2_000_000_000)
        generate_video_auto(image_url, audio_local_path, out_path, prompt, resolution, attempt_seed, chunk_dir)
...
# Run #1 (2026-08-23), the first real full-video execution: a real "1080p"
# request to wavespeed-ai/ltx-2.3/lipsync came back 1024x1920, not 1080x1920
# (5.19% off literal 9:16). Investigated before touching this: WaveSpeed's own
# docs for this model name 480p/720p/1080p only as price/quality tiers, never
# promise an exact pixel width — no evidence this was a request-side mistake.
# Likely normal model behavior for this tier (not confirmed with certainty —
# no first-party spec found, and this is a SINGLE real sample). Tolerance
# below is set to cover that one real measurement plus a small margin, not
# loosened blindly — still must reject a genuinely wrong shape (e.g. an
# accidental 16:9 landscape frame). Treat 6% as provisional: revisit if a
# future real run shows a different width at the same tier, don't widen
# further without new evidence.
VIDEO_ASPECT_TOLERANCE = 0.06


def _aspect_ratio_ok(width, height, expected_aspect="9:16", tolerance=None):
    tolerance = tolerance if tolerance is not None else VIDEO_ASPECT_TOLERANCE
...
def check_generated_voice_duration(audio_path, duration_range=None):
    """Run #1 (2026-08-23) finding: check_script's pre-flight duration is
    only a word-count ESTIMATE (FRENCH_WORDS_PER_SECOND) — the real
    voice.mp3 came out at 10.55s against an estimated 16.9s, and this wasn't
    caught until the very last pipeline step (format_check), after the most
    expensive step (video generation) had already run and been paid for.
    This measures the REAL generated audio and is meant to be called right
    after generate_voice, before any video generation — fail here, fail
    cheap. Does NOT change FRENCH_WORDS_PER_SECOND itself: one real sample
    isn't enough to responsibly recalibrate that estimate."""
    dur = _audio_duration_seconds(audio_path)
    lo, hi = duration_range or SCRIPT_DURATION_TARGET_SECONDS
    ok = dur is not None and lo <= dur <= hi
    return {
        "ok": ok,
        "real_duration_seconds": round(dur, 2) if dur is not None else None,
        "target_range": [lo, hi],
    }
...
def check_video_format(video_path, expected_aspect="9:16", duration_range=None):
    ...
    the target range. Aspect ratio compared with a tolerance since exact
    pixel dimensions vary by model/resolution setting — see
    VIDEO_ASPECT_TOLERANCE above for why 6%, not an arbitrary number."""
    import imageio_ffmpeg
    import subprocess
    import re
    ffmpeg = imageio_ffmpeg.get_ffmpeg_exe()
    ...
    aspect_ok = _aspect_ratio_ok(w, h, expected_aspect)
...
def run_full_video(slug, text, scene="car", extra_photo_prompt="", emotion="happy", speed=1.1,
    ...
    if already_done("check_balance"):
        ...
        balance_check = check_balance_hard_stop_if_insufficient()
    ...
    if already_done("check_voice_duration"):
        skip("check_voice_duration")
    else:
        voice_duration = check_generated_voice_duration(paths["voice"])
        record("check_voice_duration", voice_duration["ok"], voice_duration)
    ...
    if already_done("generate_video", paths["video"]):
        ...
        video_qc = generate_video_with_qc(photo_url, paths["voice"], paths["video"], resolution=resolution, seed=seed)
