...
function pickFreeMatch(list,horloge,market){
  ...
  if(designe.length){
    // 16/09/2026 (audit du site en ligne) : jamais un match deja commence ou
    // termine comme analyse offerte. On passe a la prochaine designation ; s'il
    // n'y en a plus, aucun match offert (jamais un match non designe, dont
    // l'analyse n'est pas publiee en clair).
    designe=designe.filter(function(m){ return L.cle(m)>t.now; });
