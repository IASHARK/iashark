...
function calcFinalProbs(lambdaH, lambdaA, eloStats) {
  var poissonMatrix=buildPoissonMatrix(lambdaH,lambdaA,10);
  var dixonMatrix=buildDixonColesMatrix(lambdaH,lambdaA,10);
  var matrix=blendMatrices([{matrix:poissonMatrix,weight:0.35},{matrix:dixonMatrix,weight:0.65}]);
  ...
  var mc = calcMonteCarlo(lambdaH, lambdaA);
  return {
