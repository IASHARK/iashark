...
// conf (teaser public) retombe sur la qualite des donnees, comme un match sans
// pari. Mutation en place ; renvoie le match.
function closeMatchForPick(match, premiumRow, premiumFields, reason) {
  ...
  match.is_free = false;
  const dq = Number(match.data_quality_score);
  match.conf = Number.isFinite(dq) ? Math.round((dq / 10) * 10) / 10 : 0;
