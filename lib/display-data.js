(function(root,factory){
  ...
  function sourceLabels(match){
    // Aucun nom de fournisseur (decision du proprietaire, audit du 18/09/2026).
    var labels=['Calendrier et équipes'];
    if(hasReliableCriteria(match&&match.crit_home,match&&match.crit_away))labels.push('Statistiques des équipes');
    if(hasReliableEventPatterns(match&&match.events_home,match&&match.events_away))labels.push('Événements des matchs précédents');
    if(match&&Array.isArray(match.injuries)&&match.injuries.length)labels.push('Blessures et suspensions');
    if(match&&match.market_source&&match.market_source!=='Aucune cote fiable')labels.push('Cotes du marché');
    return labels;
  }
