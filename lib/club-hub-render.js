...
// ---------------------------------------------------------------------------
// Blocs de donnees.
// Probabilite estimee (conf) : seulement pour le match offert du jour. Un match
// payant garde "analyse publiee" + lien, sans aucun chiffre du modele.
function showConf(fx) { return !!(fx && fx.analysis && fx.analysis.isFree === true && fx.analysis.conf != null); }
...
function fixtureItem(fx, ctx) {
  ...
  if (fx.analysis) {
    ...
    act = '<a href="' + (hasStatic ? staticPath : appPath) + '">' + esc(L.analysis_link) + "</a>" +
      ...
      (showConf(fx) ? '<span class="conf">' + esc(L.conf_label) + " " + esc(numText(fx.analysis.conf, dir, 1)) + "/10</span>" : "");
  } else {
...
function fixturesBlock(list, ctx, noneText) {
  ...
  var anyConf = list.some(showConf);
