...
- Testée en direct par requête HTTP réelle : reçoit un événement, répond `{"ok":true,"payment_provider":"disabled","processed":false}`, **aucune ligne écrite** dans `billing_customers`/`subscriptions`/`billing_events` (vérifié par requête SQL directe après le test).
- Le flag s'appelle `PAYMENT_PROVIDER` (pas `BILLING_MODE`) : architecture volontairement agnostique du prestataire — Stripe est l'implémentation concrète déjà câblée, mais le choix définitif du prestataire est une décision séparée qui ne bloque pas la V2 (voir `IASHARK_V2_EXECUTION_STATE.md`).
...
4. **Ajouter les secrets à Supabase** (dashboard projet → Edge Functions → Secrets, ou `supabase secrets set`) : `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, et `PAYMENT_PROVIDER=stripe` (bascule le webhook de désactivé à actif — **avant cette étape, le webhook ne traite jamais rien, quoi qu'il reçoive**).
...
8. **Une fois tout vérifié en mode test** : basculer les clés Stripe de test vers les clés live, garder `PAYMENT_PROVIDER=stripe`.
...
- La distinction `PAYMENT_PROVIDER=test` (mentionnée par le MASTER comme mode intermédiaire) n'est pas implémentée séparément de `stripe` dans le code actuel — seule la bascule binaire désactivé/activé existe. À affiner si un mode "test visible mais non payant" est souhaité en plus du mode test Stripe lui-même.
- Le choix du prestataire de paiement final (Stripe vs autre) est une décision séparée de l'utilisateur, explicitement reportée — ne bloque pas la V2.
