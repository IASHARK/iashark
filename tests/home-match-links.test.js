...
test("accueils de version : chaque lien du resume vise une page match existante, de la version ou de la version la plus proche", () => {
  ...
  for (const dir of C.DIR_CODES) {
    ...
    for (const { href, id } of summaryLinks(html)) {
      ...
      assert.equal(href, L.versionMatchHref(id, null, dir, ROOT), dir + " : " + href + " n'est pas la version la plus proche");
    }
  }
...
test("build-locales : meme resolution que le pipeline (version, meme langue, en, fr ; sinon nom sans lien)", () => {
  ...
  try {
    ...
    const expected = {
      ...
      gb: ["/en/match/101.html", "/match/202.html"],
      za: ["/za/match/101.html", "/match/202.html"],
      en: ["/en/match/101.html", "/match/202.html"],
      // Premier League sans version espagnole : en (preference x-default) avant fr.
      mx: ["/en/match/101.html", "/es/match/202.html"],
    ...
    for (const dir of Object.keys(expected)) {
      ...
      assert.deepEqual(summaryLinks(out).map((l) => l.href), expected[dir], dir);
...
test("pipeline : injectHomeSeoSummary utilise la meme resolution que build-locales", () => {
  ...
  assert.match(wf, /var lienMatch=MATCH_LIFECYCLE\.versionMatchHref\(m\.id,m\.league_key,dir\|\|'fr','\.'\);/);
