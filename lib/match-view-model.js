(function (root, factory) {
  ...
  function marketTable(raw, marketsCompared, recommendation) {
    ...
    add({ id: 'home-win', label: 'Victoire domicile', group: 'result', model: round1(raw.p1), market: round1(raw.market_consensus_p1), odds: finite(raw.c1) });
    add({ id: 'draw', label: 'Match nul', group: 'result', model: round1(raw.pn), market: round1(raw.market_consensus_pN), odds: finite(raw.cn) });
    add({ id: 'away-win', label: 'Victoire exterieur', group: 'result', model: round1(raw.p2), market: round1(raw.market_consensus_p2), odds: finite(raw.c2) });
    add({ id: 'over-25', label: 'Over 2.5', group: 'goals', model: round1(raw.po25), market: demargined(raw.co25, raw.cu25), odds: finite(raw.co25) });
    add({ id: 'btts-yes', label: 'BTTS Oui', group: 'goals', model: round1(raw.btts), market: demargined(raw.cbtts, raw.cbtts_non), odds: finite(raw.cbtts) });
    (marketsCompared || []).forEach(m => {
      ...
      add({ id: m.id, label: m.market, group, model: m.probability, market: m.consensus, edge: m.edge, odds: null });
    });
    if (recommendation) {
      const rf = fam(recommendation.market);
      const odds = finite(raw.cote_rec);
      const implied = odds !== null && odds > 1 ? round1(100 / odds) : null;
  ...
  function buildMatchViewModel(raw) {
    ...
    const recommendation = modelAvailable && text(marketLabel) ? {
      market: marketLabel, probability: round1(raw.model_probability), confidence: round1(raw.conf), reliability: text(raw.reliability && raw.reliability.label)
    } : null;
    ...
    const recImplied = recOdds !== null && recOdds > 1 ? round1(100 / recOdds) : null;
