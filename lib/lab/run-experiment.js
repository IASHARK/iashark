...
const { lossDelta } = require("./loss-delta.js");
...
// du bootstrap par blocs apparies (lib/lab/bootstrap.js). M1 est le
// CANDIDAT, M0 le CHAMPION : delta = lossDelta(NLL_M1, NLL_M0) suit la
// convention officielle unique (lib/lab/loss-delta.js) - candidat
// meilleur -> delta < 0.
...
function runExperiment(options) {
  ...
  // Repartition par saison (rapport EXP-001 §11 : metriques "par saison et
  // global") - calculee sur les MEMES predictions deja produites, jamais
  // un second run walk-forward.
  ...
  return {
    ...
    rho_median: median(rhoValues),
    rho_std: rhoStd,
    rho_min: rhoValues.length ? Math.min(...rhoValues) : null,
    rho_max: rhoValues.length ? Math.max(...rhoValues) : null,
    rho_p05: percentile(rhoValues, 0.05),
    rho_p95: percentile(rhoValues, 0.95),
