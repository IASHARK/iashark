...
## Duration is decided by content, not by the test scripts (corrected 2026-08-23)

The first real scripts this project generated (~8-12s each) were deliberately
short — built to cheaply A/B test voice and video parameters, not written as
real-content targets. Don't let that become the default. `SCRIPT_DURATION_
TARGET_SECONDS` in `pipeline.py` is `(14, 50)` real seconds — pick where in
that range a script belongs based on what it actually needs to say, never by
defaulting to the shortest end because that's what the test clips looked like.

**Three tiers, chosen by content — not a menu to pick from arbitrarily:**
- **14-20s** — a short reaction or one simple piece of information.
- **20-35s** — a short piece of content built around one real, developed idea.
- **35-50s** — a subject that genuinely needs more context or analysis to land.

**Never pad and never cut to hit a number.** A script that fully makes its
point at 16s should not be stretched to 25s, and a real idea that needs 30s to
develop shouldn't be chopped down to fit a shorter tier. Duration is a
consequence of what the content needs, decided in the pre-script method below,
not a target written to independently.

## Vary hooks — don't open every video the same way (corrected 2026-08-23)

Earlier scripts used "Bon," to open almost by default — a genuine finding
from voice A/B testing (a single, sparing discourse marker reads as more
conversational than none), but it became a repeated tic instead of one option
among several. **Don't open scripts with "Bon,", "Alors," or any other fixed
marker by default.** A hook is chosen for the specific content, not recycled
from the last script. Valid openings, chosen by what actually fits:
...
- une situation vécue
- une question
- une phrase directe qui donne envie d'écouter la suite

The goal is natural variety — Shanon shouldn't sound like she opens every
video with the same formula. Same discipline as step 13 below (same-day
coherence): checked at write time, not caught after the fact.

## Pre-script method for every real content (corrected 2026-08-23)

Before writing a single line, answer these five in order — the concrete,
mandatory version of steps 1-2 below, made explicit because it was being
skipped in practice in favor of jumping straight to sentences:

1. **Le sujet réel** — what this video is actually about.
2. **L'information ou l'idée principale** — the one thing the video needs to
   land.
3. **Ce qui rend le début intéressant** — the specific hook, chosen from the
   list above, not defaulted to.
4. **La durée appropriée au sujet** — which of the three tiers above this
   content actually needs.
5. **L'énergie et le contexte de performance correspondants** — feeds
   directly into `video-performance-protocol.md`'s four questions.

Only after these five are answered does sentence-writing start.
