...
(function (root, factory) {
  ...
  var LANG_DIR = { fr: "fr", en: "en", es: "es", de: "de", it: "it", pt: "pt" };
  ...
  // Offre (marche de config/markets.json) qui s'applique a un pays : les cles
  // de marche pays sont les codes pays en minuscules (gb, za, mx) ; tout autre
  // pays releve de l'offre EUR internationale "fr".
  function countryMarket(country, dirs) {
    var c = String(country || "").toLowerCase();
    if (!c) return "";
    for (var i = 0; i < dirs.length; i++) if (dirs[i].market === c && c !== "fr") return c;
    return "fr";
  }
  ...
  function decide(env) {
    ...
    // Jamais vers une autre offre/devise sans savoir qu'elle s'applique au visiteur.
    if (t.market !== page.market) {
  ...
  return {
    decide: decide, run: run, countryMarket: countryMarket, isBot: isBot,
